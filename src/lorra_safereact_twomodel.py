...
def compute_loss(self, model, inputs, target_layers, alpha, ref_model, return_outputs=False, tokenizer=None, **kwargs):
    ...
    # ==== safe_cb =====
    safecb_input_ids = inputs.get(f"input_ids_cbsafe")
    safecb_attention_mask = inputs.get(f"attention_mask_cbsafe")
    # ==== val ====
    ...
    safecb_inputs = dict(input_ids=safecb_input_ids, attention_mask=safecb_attention_mask, output_hidden_states=True)
    ...
    retain_coeff, circuit_breaker_coeff = alpha * scheduled_coeff, alpha * (1-scheduled_coeff)#alpha * scheduled_coeff#2 alpha
    
    print(f"retain_coeff: {retain_coeff:.4f} || circuit_breaker_coeff: {circuit_breaker_coeff:.4f}")
    
    # ===== loss components =====
    layers_safecb_attention_mask = safecb_attention_mask.repeat(len(target_layers), 1, 1).unsqueeze(-1)
    ...
    with model.disable_adapter():
        ...
        with torch.no_grad():
            ...
            if circuit_breaker_coeff > 0:
                if ref_model is not None:
                    print("Aligning with ref model")
                    circuit_breaker_outputs = ref_model(**safecb_inputs)[module]
                else:
                    print("Aligning with original model")
                    circuit_breaker_outputs = model(**safecb_inputs)[module]
                circuit_breaker_hidden = torch.stack([circuit_breaker_outputs[l].detach() for l in target_layers]) * layers_safecb_attention_mask
    ...
    if circuit_breaker_coeff > 0:
        ...
        lora_circuit_breaker_hidden = torch.stack([lora_circuit_breaker_outputs[l] for l in target_layers]) * layers_circuit_breaker_attention_mask

        circuit_breaker_loss = torch.norm(circuit_breaker_hidden - lora_circuit_breaker_hidden, dim=-1, p=2, dtype=torch.float).nanmean()
        if circuit_breaker_loss.isnan():
            circuit_breaker_loss = 0
        

        cb_cosine = cosine_similarity(circuit_breaker_hidden, lora_circuit_breaker_hidden, dim=-1) * layers_circuit_breaker_attention_mask.squeeze(-1)
        with model.disable_adapter():
            ori_circuit_breaker_outputs = model(**cb_inputs)[module]
            ori_circuit_breaker_hidden = torch.stack([ori_circuit_breaker_outputs[l].detach() for l in target_layers]) * layers_circuit_breaker_attention_mask
        cb_cosine_ori = cosine_similarity(ori_circuit_breaker_hidden, lora_circuit_breaker_hidden, dim=-1) * layers_circuit_breaker_attention_mask.squeeze(-1)
        print(f"\ncircuit_cos_sim: {(cb_cosine.sum() / layers_circuit_breaker_attention_mask.sum()).item():.4f}")
        print(f"\ncircuit_cos_sim_ORI: {(cb_cosine_ori.sum() / layers_circuit_breaker_attention_mask.sum()).item():.4f}")
...
def get_model_generation(inputs, model, tokenizer, prefill=""):
    ...
    with torch.no_grad():
        ...
        print("Model Generation", sanity_generation)
...
def train():
    ...
    refmodel_path = model_args.refmodel_path
    ...
    train_dataset = CircuitBreakerDataset(tokenizer, num_examples=800, lorra_args=lorra_args, model_name_or_path=model_name_or_path)
    ...
    class CustomTrainer(Trainer):
        ...
        def get_training_progress(self):
            return self.current_training_step / (self.args.max_steps * 1.5)
